...
use marmot_app::{
    AccountRelayListStatus, AppError, AppGroupRecord, AppMessageQuery, AppMessageRecord, AppStatus,
...
use transport_quic_stream::{AgentTextStreamCrypto, ServerTrust};
...
pub(crate) use commands::stream::StreamCommand;
...
pub(crate) const AGENT_STREAM_START_LOOKBACK_LIMIT: usize = 200;
...
async fn execute_inner(cli: Cli) -> Result<CommandOutput, DmError> {
    ...
    {
        return commands::stream::run_local(command.clone()).await;
    }
    ...
    {
        return commands::stream::run_local(stream_command.clone()).await;
    }
    ...
    match command {
        Command::Debug { command } => {
            commands::debug::run(&account_home, &app, command, account_flag)
        }
        ...
        Command::Groups { command } => {
            commands::groups::run(&account_home, &app, command, account_flag).await
        }
        Command::Message { command } => {
            commands::message::run(&account_home, &app, command, account_flag).await
        }
        Command::Messages { command } => {
            commands::message::run(&account_home, &app, command, account_flag).await
        }
        Command::Follows { command } => {
            commands::follows::run(&account_home, &app, command, account_flag, relay).await
        }
        Command::Profile { command } => {
            commands::profile::run(&account_home, &app, command, account_flag, relay).await
        }
        Command::Relays { command } => {
            commands::relays::run(&account_home, &app, command, account_flag, relay).await
        }
        ...
        Command::Stream { command } => {
            commands::stream::run(&account_home, &app, command, account_flag).await
        }
